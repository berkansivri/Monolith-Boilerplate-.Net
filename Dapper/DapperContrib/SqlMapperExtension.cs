    ...
    public static partial class SqlMapperExtensions
    {
        ...
        public static T Get<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (!GetQueries.TryGetValue(type.TypeHandle, out string sql))
            {
                ...
                sql = $"select * from {name} where {(key.GetCustomAttribute(typeof(DescriptionAttribute)) as DescriptionAttribute).Description} = :id";
        ...
        public static long Insert<T>(this IDbConnection connection, T entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            for (var i = 0; i < allProperties.Count; i++)
            {
                ...
                adapter.AppendColumnName(sbColumnList, (property.GetCustomAttribute(typeof(DescriptionAttribute)) as DescriptionAttribute).Description);
            ...
            int returnVal = adapter.Insert(connection, transaction, commandTimeout, name, sbColumnList.ToString(),
                sbParameterList.ToString(), keyProperties, entityToInsert);
        ...
        public static bool Update<T>(this IDbConnection connection, T entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            for (var i = 0; i < nonIdProps.Count; i++)
            {
                ...
                adapter.AppendColumnNameEqualsValue(sb, (property.GetCustomAttribute(typeof(DescriptionAttribute)) as DescriptionAttribute).Description + "æ" + property.Name); if (i < nonIdProps.Count - 1)
                    sb.Append(", ");
            }
            sb.Append(" where ");
            for (var i = 0; i < keyProperties.Count; i++)
            {
                ...
                adapter.AppendColumnNameEqualsValue(sb, (property.GetCustomAttribute(typeof(DescriptionAttribute)) as DescriptionAttribute).Description + "æ" + property.Name); if (i < keyProperties.Count - 1)
                    sb.Append(" and ");
            }
        ...
        public static bool Delete<T>(this IDbConnection connection, T entityToDelete, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            {
                var property = keyProperties[i];
                adapter.AppendColumnNameEqualsValue(sb, (property.GetCustomAttribute(typeof(DescriptionAttribute)) as DescriptionAttribute).Description + "æ" + property.Name);
...
public partial class OracleAdapter : ISqlAdapter
{
    ...
    public int Insert(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, string tableName, string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert)
    {
        ...
        if (entityToInsert is IEnumerable)
        {
            FillEnumerableObjectId(connection, transaction, commandTimeout, tableName, keyProperties, entityToInsert);
            int count = connection.Execute(cmd, entityToInsert, transaction, commandTimeout);

            return count;
        }
        else
        {
            int id = FillSingleObjectId(connection, transaction, commandTimeout, tableName, keyProperties, entityToInsert);
    ...
    public int FillSingleObjectId(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, string tableName, IEnumerable<PropertyInfo> keyProperties, object entityToInsert)
    {
        ...
        if (propertyInfos.Length > 0)
        {
            ...
            id = Convert.ToInt32(connection.ExecuteScalar($"SELECT \"sq_{tableName}\".nextval FROM DUAL", transaction: transaction, commandTimeout: commandTimeout));
            idProperty.SetValue(entityToInsert, Convert.ChangeType(id, idProperty.PropertyType), null);
        }
        return id;
    }

    public void FillEnumerableObjectId(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, string tableName, IEnumerable<PropertyInfo> keyProperties, object entityToInsert)
    {
        var propertyInfos = keyProperties as PropertyInfo[] ?? keyProperties.ToArray();
        int id = 0;
        if (propertyInfos.Length > 0)
        {
            var idProperty = propertyInfos[0];
            foreach (var item in entityToInsert as IEnumerable)
            {
                id = Convert.ToInt32(connection.ExecuteScalar($"SELECT \"sq_{tableName}\".nextval FROM DUAL", transaction: transaction, commandTimeout: commandTimeout));
                idProperty.SetValue(item, Convert.ChangeType(id, idProperty.PropertyType), null);
            }
        }
    }
    ...
    /// <param name="sb">The string builder  to append to.</param>
    /// <param name="columnName">The column name.</param>
    public void AppendColumnNameEqualsValue(StringBuilder sb, string ColumnAndParameterName)
    {
        sb.AppendFormat("{0} = :{1}", ColumnAndParameterName.Split('æ')[0], ColumnAndParameterName.Split('æ')[1]);
    }
}
